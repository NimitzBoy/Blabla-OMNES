...
if (isset($_GET['id_trajet']) && isset($_GET['id_passager'])) {
    ...
    try {
        ...
        $stmt = $bdd->prepare("UPDATE reservation SET statut = 'refusé' WHERE id_trajet = :id_trajet AND id_passager = :id_passager");
        $stmt->bindParam(':id_trajet', $id_trajet);
        $stmt->bindParam(':id_passager', $id_passager);
        $stmt->execute();

        // Ajouter une notification pour le passager
        $notification = "Votre réservation pour le trajet n° " . $id_trajet . " a été refusée.";
        ...
        echo "<h2>Réservation refusée avec succès.</h2>";
    } catch (Exception $e) {
